...
export default class ListView extends Component {
  ...
  componentDidMount(){
    database.ref().child('events/3/eventdetails').on('value', (snapshot)=> {
      const lectures = [];
       ...
       <FlatList
        ...
        renderItem={({item}) =>
            ...
            <View style={{flex:1, flexDirection: 'row'}}>
              ...
              <Button buttonStyle={{backgroundColor: '#ff4d4d', width: 100, height:30}} title='Follow' onPress={()=>this.getfavoriteEvents(item.id, item.title, item.description, item.date, item.image)} />
              <Spacer size={15} />
              </View>
            </View>
